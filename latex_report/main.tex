...
a series of Convolutional and Pooling layers. The final prediction is calculated in the final
fully connected (Dense) layer shown on the right-hand side of Figure~\ref{fig:ccpm}. The number
of maximum features that the intermediate pooling layers take is flexible to account for the
flexible input matrix length.
...
input matrix \cite{RefWorks:zhang2021deep,RefWorks:qu2018product-based,RefWorks:gu2021ad}. Convolutions
by their extract feature maps in the local neighbourhood of each variable, but fail to do so globally.
