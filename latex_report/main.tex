...
Figure~\ref{alg:drl-ap} shows the resulting episodic Cumulative Regret and 5-episode moving average
Click Through rate for the simulation described in section~\ref{sec:sim-description}. The results
for the DRL-AP algorithm are shown in blue. As a benchmark, we simulated the results for a random
model. In this case, the ads we chosen from $\mathcal{A}_t$ uniformly at random. The mean result
for the random model is represented by the orange line in both plots. Figure~\ref{fig:drl-ap}
clearly shows that the DRL-AP algorithm minimizes regret and therefore maximizes CTR better than
the random model. This indicates that the predictive precision DCN model as a Q-function approximator
significantly improves the Ad selection agents ability to choose adverts that the user will click on,
which demonstrates the proof-of-concept for the DRL-AP algorithm.
