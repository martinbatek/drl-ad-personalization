...
Figure~\ref{alg:drl-ap} shows the result of the reinfocement learning simulation
described in section~\ref{sec:sim-description} over 30 episodes. The left-hand pane shows
the simulation Regret, calculated cumulatively as in equation~\ref{eqn:sim-regret}.
The middle pane shows the cumulative number of clicks by the end of each episode.
The right-hand pane shows the 5-episode moving average Click-Through Rate. The underlying
data for Figure~\ref{fig:drl-ap} is included in the appendix in Tables \ref{tab:drl-ap},
\ref{tab:random-selection} and \ref{tab:dcn-model}.

The results for the DRL-AP algorithm (as outlined in Algorithm~\ref{alg:drl-ap}) are shown in blue. 
