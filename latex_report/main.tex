...
the Logistic Regression model, as implemented by \cite{RefWorks:richardson2007predicting} on
...
\begin{equation}
\label{eqn:lr-logit}
\log \left(\frac{p(y)}{1 - p(y)}\right) = \theta_0 + \sum_{j=1}^{\tilde{n}}\theta_j \tilde{x}_j
\end{equation}

Equation~\ref{eqn:lr-logit} can be expressed in terms of $p(y)$, in order to
recover the same formulation as in equation~\ref{eqn:ctr-classifier}:

\begin{equation}
    \label{eqn:lr-model}
    p(y) = \frac{1}{1 + e^{- \theta_0 - \sum \theta_j \tilde{x}_j}}
    = f_{\Theta}^{LR}(\tilde{\mathbf{x}})
\end{equation}

where $f_{\Theta}^{LR}: \mathbb{R}^n \Rightarrow [0,1]$ represents the Logistic regression
...
Howevever, the formulation in equation~\ref{eqn:lr-logit} reveals that the LR model only
accounts for the individual feature values as they are, and none of the \emph{feature interactions}.
