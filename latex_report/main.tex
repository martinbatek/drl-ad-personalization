...
by the chosen embedding dimension $D$ and the cardinality of the feature, $C_i$. 
...
\begin{equation}
    \begin{split}
    \dot{\mathbf{x}} &= \left[ \mathbf{e}_1^{\intercal}, \dots , \mathbf{e}_s^{\intercal}, z_{s+1}, \dots , z_{s+d} \right]
    \end{split}
\end{equation}

where $\dot{n} = D\cdot s + d$
...
an embedding layer, a unique type of product layer called a \emph{Bi-Interaction Layer}
...
Figure~\ref{fig:nfm} reveals that the structure of the network represented by $f_{\Phi}^{NFM-Deep}$
is similar to that of PNN as shown in Figure~\ref{fig:pnn}. The key difference is
that rather than using a Inner or Outper product layer, NFM instead uses a Bi-Interaction Layer
that 
