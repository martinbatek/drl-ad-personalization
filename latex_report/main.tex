...
Survival Analysis \citep{ibrahim2001bayesian,jing2017neural}. Secondly rather than applying the
$\epsilon$-greedy method for exploration, DRN uses the \textbf{Dueling Bandit Gradient Descent} algorithm.
This algorithm proceeds by generating an \emph{explore network} $\tilde{Q}$ (a copy of the regular 
\emph{exploit network} $Q$, with it's weights randomly perturbed) before each timestamp $t_i$, and
then by randomly choosing either $Q$ or $\tilde{Q}$ to determine the next action. If $\tilde{Q}$
outperforms $Q$ after a set number of actions, $\tilde{Q}$ is then adoped as the \emph{exploit network}.
...
\begin{enumerate}
    ...
    The agent then selects the top $L$ candidantes. The list of $L$ candidates is compiled by mixing
    ...
    Algorithm~\ref{alg:deep-q-learning}.
\end{enumerate}
