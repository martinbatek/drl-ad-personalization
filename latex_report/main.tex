...
All of the preprocessing steps discussed in section~\ref{sec:datasets-preprocessing} including
categorical value binning and encoding, numberical value normalization and target label weighting
apply here as well, and these steps have therefore been encoded as in the preprocessing functions
$\phi$ used for the model pretraining for the Offline Part, as well as the Online Algorithm.
...
for the environment). For each session $t$ in the simulation, we ``observe'' the
...
The hyperparameter settings for the RL simulations have been chosen arbitrarily
