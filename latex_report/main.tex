...
The results for experiment~\ref{exp:comparative-model-analysis} are visualized
in Figure~\ref{fig:experiment-1} (the metrics are are also available in the appendix
in tables \ref{tab:exp-1-kdd12}, \ref{tab:exp-1-avazu} and \ref{tab:exp-1-criteo}). Only the
metrics for the epochs with the minimal training loss function (Binary Crossentropy) are 
reported. The left-hand panes show plots of Binary Crossentropy
vs AUC for all of the models, the middel panes show Precision vs Recall and the right
hand panes show the number of Epochs before the lowest Loss value was reached during
training for each of the model.
...
When examining the Shallow model vs Deep model aspect of the metrics
in Figure~\ref{fig:experiment-1}, we find that although the Logistic
...
they outperformed some of their DNN counterparts (such as in the case of
...
The Binary Crossentropy vs AUC reveal that the \textbf{Deep and Cross Network} (DCN)
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../figures/cross_layers.png}
...
otherwise largely based in DRN algorithm. We then conduct an offline simulation using the KDD12
dataset introduced in chapter~\ref{chap:deep-ctr-model-evaluation} as a proof-of-concept for DRL-AP.
The pretraining step of the DRL-AP algorithm is implemented in the Python Notebook \verb|rl_model_pretraining.ipynb|,
...
be included in future iterations. The DRL-AP algorithm is summarized in \ref{alg:drl-ap}.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \For{Episode $k=1,\ldots, K$}
            \State Initialize state sequence $s_1$ as a combination of user $u_1$ and context/query $c_1$ features
            \State Preprocess the sequence and set as state values for the first session $\{\phi_h\}_{h=1}^P = \phi(s_1)$
            ...
            \State \textbf{FEEDBACK:} Recieve feedback from the user in the form of clicks $\{r_h\}_{h=(t-1)P+1}^{tP}$
            ...
            \State Perform gradient descent step on $\mathcal{L}(\Theta) = \left(y_j - f_{\Theta}(\phi_j,a_j)\right)^2$ with respect to $\Theta$
            \State Every $C$ steps reset the target action-value function $\hat{f} = f$ by setting weights $\hat{\Theta} = \Theta$
            \EndFor
        \EndFor
    \end{algorithmic}
